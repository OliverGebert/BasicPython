...
while snake.alive:      # repeat until termination evant is sent
    ...
    if snake.collision(food.position()):
        ...
        food.new_position()
        sec -= 0.2 / (2 ** (snake.length()-1))              # assymptotisch gegen Null (initial value) -1/2, -1/4, -1/8, etc.
        scr_brd.update(f"Score: {snake.length()} Sleep: {sec:0.2f}", "white")
