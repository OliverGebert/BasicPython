...
def markov(text):
    ...
    for (
        key,
        value,
    ) in md.items():  # Schleife über alle Elemente in WortHistogramm mit key + value
        hl.append(
            (key[0], key[1], value)
        )  # tausche key und value und hänge sie an das Dictionary für die Worthäufigkeit an
...
def extrahiere_liste(sl, pattern):
    ...
    rd = {}
    for key1, key2, value in sl:  # iteriere über dreier Tupel in der Liste
        if key1 == pattern:
            rd[key2] = value
    return rd
...
def textgenerator(ml, start, anz=50):
    ...
    for i in range(anz):
        d = extrahiere_liste(
            ml, start
        )  # erstelle subDictionary aus Folgewort + Häufigkeit für alle Elemente, bei denen der erste key korrekt ist
        start = zufalls_wort(
            d
        )  # generiere ein Zufallswort aus dem Dictionary gemäß Häufigkeit und setze den Startwer neu
...
with open(file) as fin:
    for line in fin:
        ...
        sub_Dict = {
            10: 32
        }  # dictionary, dass für alle Line Feed ASCI10 ein Space setzt ASCII32
        text_Block += line.translate(
            sub_Dict
        )  # erstellt Zeile für Zeile einen Textblock und ersetzt gemäß sub_Dict
        markov_List = markov(
            text_Block
        )  # erstellt eine Häufigkeitsverteilung präfix zu suffix und Häufigkeit als dreier Tupel

        exclude_Dict = str.maketrans(
            "", "", string.punctuation
        )  # erstellt ein dictionary, dass alle Sonderzeichen aus punctuation entfernt - Arg0+Arg1 ersetzen "" durch "" - der drite parameter wird zum entfernen genutzt
        zeile = (
            line.strip().lower().translate(exclude_Dict)
        )  # entfernt leerzeichen am Anfang und Ende vom String + ganzer String in lower case
        ...
        zeilen_Liste.append(
            zeile
        )  # hängt Zeile für Zeile an eine Liste von Zeilen - strip entfernt führende und endende leerzeichen
        zeilen_Wort_Liste.append(
            zeile.split()
        )  # erweitert Zeile für Zeile eine Liste von Worten zu eine 2D Liste
        text_Histogram = histogram(
            text_Histogram, zeile
        )  # übergibt texthistogram und ergänzt alle neuen Einträge der neuen übergebenen Text Zeile
        wm = set(
            zeile.split()
        )  # erstellt ein Set von Worten (wm - wortmenge) aus der aktuellen Zeile
        for word in wm:
            if word not in wort_Histogram:
                worte_Menge.add(
                    word
                )  # ergänzt die Wortmenge der aktuellen Zeile zur Gesamtmenge
                wort_Histogram[word] = 1  # Zähler für Wort histogramm auf 1
            else:
                wort_Histogram[word] += 1
            # Annagramm prüfen
            if anna(word) and len(word) > 1:
                if word not in anna_Liste:
                    anna_Liste.append(word)
                    anna_Zaehler += 1
...
for (
    ...
    wort_Histogram.items()
):  # Schleife über alle Elemente in WortHistogramm mit key + value
    wh_ValueList.append(
        (value, key)
    )  # tausche key und value und hänge sie an das Dictionary für die Worthäufigkeit an
wh_ValueList.sort(reverse=True)  # sortiere umekehrt

text_Zufall = textgenerator(
    markov_List, muster, zaehler // 2
)  # erstellt einen ZUfallstext auf Basis der Markov Verteilung und der Anzahl von Worten
...
for key in sorted(text_Histogram):  # schleife über all Buchstaben, im Histogramm
    print(
        ...
        text_Histogram[key],
        "\t\t: ",
        float(text_Histogram[key] / sum(text_Histogram.values()) * 100),
    )

print(CGREEN + "\nAnzahl der Worte ingesammt: ", sum(wort_Histogram.values()))
print("Anzahl der verschiedenen Worte in der Datei: ", len(wortliste))
print("Liste aller Worte Top 10: \n")  # \ Shift + Alt + 7
...
print(CYELLOW + "Annagramm Liste: \n", trennzeichen.join(anna_Liste[:]), CEND)
